...
public class MenuRouterBuilder {
     ...
     * @return 路由树
     */
    public static List<RouterVO> buildMenuTree(List<SysMenus> menus) {
        // 1. 过滤掉按钮类型（menuType = F）
        List<SysMenus> menuList = menus.stream()
                .filter(m -> !"F".equals(m.getMenuType()))
                .sorted(Comparator.comparing(SysMenus::getOrderNum, Comparator.nullsLast(Integer::compareTo)))
     ...
     * 获取组件路径
     */
    private static String getComponent(SysMenus menu) {
     ...
     * 获取前端 path 字段
     */
    private static String getRouterPath(SysMenus menu) {
